...
async function main() {
  ...
  try {
    console.log('Navigating to ImportBooking page...');
    await page.goto('https://flightvn.com/Booking/ImportBooking/?airlineId=VNA', {
      waitUntil: 'networkidle2',
      timeout: 30000
    });

    console.log('Running fetch inside page context...');
    const result = await page.evaluate(async () => {
      // We will perform a POST request using FormData (multipart/form-data)
      ...
      // Parse tables directly here using DOMParser
      ...
      // Extract the text of "#TicketFace"
      const ticketFace = doc.querySelector('#TicketFace')?.textContent || '';

      return { success: true, url: response.url, tables, ticketFace };
    });
