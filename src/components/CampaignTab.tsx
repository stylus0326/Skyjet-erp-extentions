...
export function CampaignTab({ hideExpired, isSelected }: { hideExpired?: boolean; isSelected?: boolean }) {
  ...
  const fetchAirlines = async (force = false) => {
    if (!force) {
      const cached = localStorage.getItem('skyjet_cache_airlines');
      if (cached) {
        setAirlinesList(JSON.parse(cached));
        return;
      }
    }
    try {
      ...
      if (data && data.length > 0) {
        const list = data.map((row: any) => row.iata_code);
        setAirlinesList(list);
        localStorage.setItem('skyjet_cache_airlines', JSON.stringify(list));
      }
    } catch (err) {
      console.error('Error fetching airlines:', err);
    }
            ...
            <table className="minimal-table">
              <thead>
                <tr>
                  ...
                  <th>Thẻ loại trừ đầu tiên</th>
                  <th onClick={() => handleSort('group')} className="cursor-pointer select-none">
                    <div className="flex items-center gap-1">
                      Nhóm
                      <ArrowUpDown className="w-3 h-3 text-slate-400" />
                    </div>
                  ...
                  <th onClick={() => handleSort('channel')} className="cursor-pointer select-none">
                    <div className="flex items-center gap-1">
                      Kênh
                      <ArrowUpDown className="w-3 h-3 text-slate-400" />
                    </div>
                  </th>
                  <th onClick={() => handleSort('ticket')} className="cursor-pointer select-none">
                    <div className="flex items-center gap-1">
                      CK theo
                      <ArrowUpDown className="w-3 h-3 text-slate-400" />
                    </div>
          ...
          <div className="bg-zinc-900 rounded-2xl w-full max-w-xl shadow-2xl border border-zinc-800 overflow-visible my-8 animate-in fade-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-zinc-400 mb-1">Thời hạn Hiệu lực (Từ ngày – Đến ngày)</label>
                  <CustomDateRangePicker
                    startDate={formValidFrom}
                    endDate={formValidTo}
                    onRangeChange={(start, end) => {
                      setFormValidFrom(start);
                      setFormValidTo(end);
                    }}
                  />
                </div>
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-zinc-400 mb-1">Thời hạn Khởi hành (Từ ngày – Đến ngày)</label>
                  <CustomDateRangePicker
                    startDate={formDepartureDateFrom}
                    endDate={formDepartureDateTo}
                    onRangeChange={(start, end) => {
                      setFormDepartureDateFrom(start);
                      setFormDepartureDateTo(end);
                    }}
                  />
                </div>
